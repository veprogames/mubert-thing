...
parser.add_argument("-n", "--num_samples",
    type=int,
    default=1,
    help=f"amount of tracks to generate")
...
subfolder = str(time.time()) if args.num_samples > 1 else ""
for i in range(args.num_samples):
    url = generate.get_track_by_tags(tags, pat, args.length, maxit=50, loop=args.loop)
    print(f"generated a track at: {url}")
    generate.download(url, subfolder=subfolder)